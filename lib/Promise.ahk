...
class Promise {
	...
	onCompleted(callback) {
		; AutoHotkey 2.1-alpha requires expression-bodied functions and
		; side-effect branches to produce a value.
		if ObjHasOwnProp(this, 'callbacks')
			this.callbacks.Push(callback)
		else
			nextTick(this, callback)
		return 0
		static nextTick(this, callback) {
			SetTimer(() {
				callback(this)
				return 0
			}, -1)
	 ...
	 */
	static reject(reason) => Promise((_, reject) {
		reject(reason)
		return 0
	})
	/**
	...
	static allSettled(promises) {
		...
		executor(resolve, reject) {
			res := [], count := res.Length := promises.Length
			callback := (index, val) {
				res[index] := { result: val.result, status: val.status }
				if !--count
					resolve(res)
				return 0
			}
	...
	static any(promises) {
		...
		executor(resolve, reject) {
			...
			reject2 := (index, err) {
				errs[index] := err
				if !--count {
					err := Error('All promises were rejected')
					err.errors := errs
					reject(err)
				}
				return 0
