...
InstallWebView2Runtime(webView2Dir) {
  ...
  try {
    ...
    if (RunWait(A_ComSpec ' /d /c expand.exe -F:* "' cabPath '" "' runtimeRoot '"', , "Hide") || !FileExist(runtimeDir "\msedgewebview2.exe")) {
      throw Error("Could not unpack the WebView2 runtime.")
    }
  } finally {
...
DownloadWebView2Runtime(cabPath) {
  ...
  pid := 0
  Run('curl.exe -L --fail --silent --show-error -o "' cabPath '" "' url '"', , "Hide", &pid)
  ...
  ToolTip()
  if (!FileExist(cabPath) || FileGetSize(cabPath) != totalBytes) {
    throw Error("Could not download the WebView2 runtime.")
  }
  return 0
}
...
try {
  ...
  ErrorLogger.Log("startup", details)
  if (uiInteropLastError != "" && uiInteropLastError != details) {
    details .= "`n`nLast OnError callback details:`n" uiInteropLastError
  }
  details .= "`n`nAutoHotkey: " A_AhkVersion
  details .= "`nCompiled: " (A_IsCompiled ? "yes" : "no")
  details .= "`nScript: " A_ScriptFullPath
  A_Clipboard := details
  MsgBox("Babyproofed Macros could not start its settings UI.`n`n" details, "Babyproofed Macros", "Iconx")
  ExitApp()
}
...
class SettingsManager {
  ...
  _mouseClickHandler(*) {
    ...
    this.CaptureHotkey(BuildCapturedHotkey(SubStr(A_ThisHotkey, 2)))
    return 0
  }
